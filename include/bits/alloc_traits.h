...
public:
    ...
    _GLIBCXX_ALLOC_TR_NESTED_TYPE(
        void_pointer, typename pointer_traits<pointer>::template rebind<void>)
    ...
    _GLIBCXX_ALLOC_TR_NESTED_TYPE(
        difference_type, typename pointer_traits<pointer>::difference_type)
...
#undef _GLIBCXX_ALLOC_TR_NESTED_TYPE
  ...
  private:
    ...
    template <typename _Alloc2> struct __select_helper {
      template <
          typename _Alloc3,
          typename = decltype(std::declval<_Alloc3 *>()
