...
namespace std _GLIBCXX_VISIBILITY(default) {
  ...
  template <typename _Tp, typename _Alloc> struct _Vector_base {
    ...
    struct _Vector_impl : public _Tp_alloc_type {
      ...
      _Vector_impl(_Tp_alloc_type &&__a) noexcept
          : _Tp_alloc_type(std::move(__a)), _M_start(0), _M_finish(0),
            _M_end_of_storage(0) {}
#endif
  ...
  private:
    // *调用_M_allocate创建一块__n大小的内存，然后_M_start指向这块内存的头部，
    // *finish指向头部，_M_end_of_storagte指向分配内存的尾部
  ...
  public:
     ...
     *  %vector's current size the %vector is truncated, otherwi se
...
#else
    template <typename _Ptr> _Ptr _M_data_ptr(_Ptr __ptr) const {
      return __ptr;
    }
#endif
  };
