...
namespace __gnu_cxx _GLIBCXX_VISIBILITY(default) {
  ...
  __pool_alloc_base::_Obj *volatile *__pool_alloc_base::_M_get_free_list(
      size_t __bytes) throw() {
  ...
  char *__pool_alloc_base::_M_allocate_chunk(size_t __n, int &__nobjs) {
    ...
    if (__bytes_left >= __total_bytes) {
      ...
      size_t __bytes_to_get =
          (2 * __total_bytes + _M_round_up(_S_heap_size >> 4));
      __try {
        // *分配失败的情况
      ...
      __catch(const std::bad_alloc &) {
        ...
        // *分配失败时，从当前的大小的__free_list开始向更大size的链表中寻找合适大小的块切割后分配。
        size_t __i = __n;
